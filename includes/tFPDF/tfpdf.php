...
function Header()
{
	// Styles top of document
    $this->SetFillColor(0, 0, 0);
    $this->Rect(0, 0, $this->w, 35, 'F');
    $this->SetFillColor(206, 222, 2);
    $this->Rect(0, 35, $this->w, 2, 'F');

	// Draws business logo
	$this->Image('/home/sistemaslmk/public_html/reportes/includes/logo.png', null, 8, 50);

    // Writes business and report titles
    $this->SetTextColor(255, 255, 255);
    $this->SetFontSize(28);
    $this->Write(8, '                    Lúmika Brillante Solución S.A. de C.V.');
    $this->Ln();
    $this->SetFontSize(20);
    $this->Write(14, '                            ' . $this->title);
	
	// Writes page number
	$this->SetXY(-22, 9);
	$this->SetFont('Arial', '', 12);
	$this->AliasNbPages();
	$this->Cell(10, 6, $this->PageNo() . '/{nb}', 0, 0, 'R');
	$this->Ln(34);
}

function Footer()
{
	// Styles bottom of document
	$this->SetFillColor(206, 222, 2);
	$this->Rect(0, $this->h - 2, $this->w, 2, 'F');
}
